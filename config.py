...
class ConfigManager:
    ...
    SESSION_EXPIRE_TIME = (
        24 * 3600
    )  # 会话过期时间（秒） - 保留向后兼容，实际使用配置中的SESSION_TIMEOUT
    ...
    def __init__(self, config_dir="."):
        ...
        # 默认日志配置
        self.logging_config = {"LOG_LEVEL": "INFO", "LOG_FILE": "lan_file_server.log"}

        # 默认主题配置
        self.theme_config = {"DEFAULT_THEME": "light"}
        ...
        self.sessions = (
            {}
        )  # 存储活跃会话：session_id -> {username, created_at, last_access, device_info, media_active}
    ...
    def _start_config_watch(self):
        ...
        self.observer.schedule(
            self.config_handler, str(self.config_dir), recursive=False
        )
        self.observer.start()
            ...
            for key, rule in rules.items():
                if key in section_data[section_name]:
                    ...
                    if not isinstance(value, rule["type"]):
                        print(
                            f"配置验证警告: {section_name}.{key} 类型错误，应为 {rule['type'].__name__}，使用默认值"
                        )
                    ...
                    if "min" in rule and value < rule["min"]:
                        print(
                            f"配置验证警告: {section_name}.{key} 小于最小值 {rule['min']}，使用默认值"
                        )
                        del section_data[section_name][key]
                        continue
                    ...
                    if "max" in rule and value > rule["max"]:
                        print(
                            f"配置验证警告: {section_name}.{key} 大于最大值 {rule['max']}，使用默认值"
                        )
                        del section_data[section_name][key]
                        continue
                    ...
                    if "allowed_values" in rule and value not in rule["allowed_values"]:
                        print(
                            f"配置验证警告: {section_name}.{key} 值无效，允许值: {rule['allowed_values']}，使用默认值"
                        )
                        del section_data[section_name][key]
                        continue
    ...
    def _create_default_json_config(self):
        ...
        return {
            ...
            "theme": {"default_theme": self.theme_config["DEFAULT_THEME"]},
            "caching": {
    ...
    def _load_json_config(self):
        ...
        try:
            ...
            if "server" in config_data:
                server_config = config_data["server"]
                self.server_config["PORT"] = server_config.get(
                    "port", self.server_config["PORT"]
                )
                self.server_config["MAX_CONCURRENT_THREADS"] = server_config.get(
                ...
                self.server_config["SSL_CERT_FILE"] = server_config.get(
                    "ssl_cert_file", self.server_config["SSL_CERT_FILE"]
                )
                self.server_config["SSL_KEY_FILE"] = server_config.get(
                    "ssl_key_file", self.server_config["SSL_KEY_FILE"]
                )
                self.server_config["FAILED_AUTH_LIMIT"] = server_config.get(
                    "failed_auth_limit", self.server_config["FAILED_AUTH_LIMIT"]
                )
                self.server_config["FAILED_AUTH_BLOCK_TIME"] = server_config.get(
                    "failed_auth_block_time",
                    self.server_config["FAILED_AUTH_BLOCK_TIME"],
                )
                self.server_config["SESSION_TIMEOUT"] = server_config.get(
                    "session_timeout", self.server_config["SESSION_TIMEOUT"]
                )
            ...
            if "logging" in config_data:
                ...
                self.logging_config["LOG_LEVEL"] = logging_config.get(
                    "log_level", self.logging_config["LOG_LEVEL"]
                ).upper()
                self.logging_config["LOG_FILE"] = logging_config.get(
                    "log_file", self.logging_config["LOG_FILE"]
                )
            ...
            if "theme" in config_data:
                ...
                self.theme_config["DEFAULT_THEME"] = theme_config.get(
            ...
            if "caching" in config_data:
                ...
                self.caching_config["INDEX_CACHE_SIZE"] = caching_config.get(
                    "index_cache_size", self.caching_config["INDEX_CACHE_SIZE"]
                )
                self.caching_config["SEARCH_CACHE_SIZE"] = caching_config.get(
                    "search_cache_size", self.caching_config["SEARCH_CACHE_SIZE"]
                )
                self.caching_config["UPDATE_INTERVAL"] = caching_config.get(
                    "update_interval", self.caching_config["UPDATE_INTERVAL"]
                )
                # 加载SQLite索引配置
                self.caching_config["ENABLE_SQLITE_INDEX"] = caching_config.get(
                    "enable_sqlite_index", self.caching_config["ENABLE_SQLITE_INDEX"]
                )

            # 加载认证配置
            if "auth" in config_data:
                auth_config = config_data["auth"]
                self.auth_config["username"] = auth_config.get(
                    "username", self.auth_config["username"]
                )
                self.auth_config["password_hash"] = auth_config.get(
                    "password_hash", self.auth_config["password_hash"]
                )
                self.auth_config["salt"] = auth_config.get(
                    "salt", self.auth_config["salt"]
                )
                # 认证配置中的失败尝试限制优先于服务器配置
                if "failed_auth_limit" in auth_config:
                    self.auth_config["failed_auth_limit"] = auth_config[
                        "failed_auth_limit"
                    ]
                    self.server_config["FAILED_AUTH_LIMIT"] = auth_config[
                        "failed_auth_limit"
                    ]
                if "failed_auth_block_time" in auth_config:
                    self.auth_config["failed_auth_block_time"] = auth_config[
                        "failed_auth_block_time"
                    ]
                    self.server_config["FAILED_AUTH_BLOCK_TIME"] = auth_config[
                        "failed_auth_block_time"
                    ]
        ...
        try:
            ...
            config_data = {
                ...
                "server": {
                    ...
                    "ssl_key_file": self.server_config["SSL_KEY_FILE"],
                    "failed_auth_limit": self.server_config["FAILED_AUTH_LIMIT"],
                    "failed_auth_block_time": self.server_config[
                        "FAILED_AUTH_BLOCK_TIME"
                    ],
                    "session_timeout": self.server_config["SESSION_TIMEOUT"],
                },
                ...
                "auth": {
                    ...
                    "salt": self.auth_config["salt"],
                    "failed_auth_limit": self.auth_config["failed_auth_limit"],
                    "failed_auth_block_time": self.auth_config[
                        "failed_auth_block_time"
                    ],
                },
                "whitelist": {
    ...
    def save_auth_config(self, username=None, password_hash=None, salt=None):
        ...
        try:
            ...
            if self.json_config_file.exists():
                ...
                config_data["auth"]["failed_auth_limit"] = self.auth_config[
                    "failed_auth_limit"
                ]
                config_data["auth"]["failed_auth_block_time"] = self.auth_config[
                    "failed_auth_block_time"
                ]
    ...
    def record_failed_attempt(self, ip_address):
        ...
        # 1. 清理过期记录（24小时过期，家庭场景无恶意攻击）
        expired_ips = []
        for ip, attempt_info in self.failed_attempts.items():
            if current_time - attempt_info["last_attempt"] > 86400:  # 86400秒=24小时
                expired_ips.append(ip)
        for ip in expired_ips:
            del self.failed_attempts[ip]

        # 2. 限制单IP最大记录数（家庭场景IP≤5，避免异常IP占用内存）
        if ip_address in self.failed_attempts:
            # 超过10次失败后，仅更新时间，不递增计数
...
if __name__ == "__main__":
    ...
    # 测试白名单检查
    test_files = ["test.jpg", "test.mp3", "test.mp4", "test.txt", "test.pdf"]
